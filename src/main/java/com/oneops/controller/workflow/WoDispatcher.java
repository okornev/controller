...
public class WoDispatcher {
     ...
     */
    public void dispatchWO(Execution ex, ExecutionContext exec, CmsWorkOrderSimple wo, String waitTaskName) {
        ...
        try {
            ...
            if (wo.rfcCi.getImpl() == null) {
                ...
                if ("class".equalsIgnoreCase(implParts[0])) {
                    ...
                    String processComplexId = processId + "!" + execId + "!" + waitTaskName;
            ...
            exec.setVariable(WorkflowController.WO_STATE, WorkflowController.WO_SUBMITTED);
        } catch (JMSException e) {
            logger.error("unable to dispactch", e);
            exec.setVariable(WorkflowController.WO_STATE, WorkflowController.WO_FAILED);
            exec.setVariable(ERROR_MESSAGE, e.getMessage());
    ...
    public void getAndDispatch(Execution exec, ExecutionContext execContext, CmsWorkOrderSimple dpmtRec) {
        try {
            CmsWorkOrderSimple wo = cmsClient.getWorkOrder(execContext, dpmtRec);
            if (wo != null) {
                cmsClient.updateWoState(exec, execContext, wo, CMSClient.INPROGRESS);
                dispatchWO(exec, execContext, wo, WAIT_STATE_NODE);
                ...
                cmsClient.updateWoState(exec, execContext, dpmtRec, CMSClient.FAILED);
